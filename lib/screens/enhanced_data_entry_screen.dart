...
/// Enhanced data entry screen for hourly readings and daily metrics
/// Integrates validation, local storage, and form persistence
class EnhancedDataEntryScreen extends ConsumerStatefulWidget {
  final String projectId;
  final String projectName;
  final DateTime selectedDate;
  final int? hour; // null for daily metrics, set for hourly readings
  final LogType logType;
  final String? existingEntryId; // for editing existing entries

  const EnhancedDataEntryScreen({
    Key? key,
  ...
  @override
  ConsumerState<EnhancedDataEntryScreen> createState() => _EnhancedDataEntryScreenState();
}

class _EnhancedDataEntryScreenState extends ConsumerState<EnhancedDataEntryScreen> {
  ...
  Future<void> _loadTemplate() async {
    try {
      ...
      final template = LogTemplateRegistry.getTemplate(widget.logType);
      setState(() {
        _template = template;
        _initializeFormData();
        _isLoading = false;
      ...
      setState(() {
        _saveError = 'Failed to load form template: $e';
        _isLoading = false;
      });
    }
  }
  ...
  Future<void> _autoSaveProgress() async {
    // Auto-save form progress to local storage (draft state)
  ...
  Future<void> _saveFormProgress() async {
    ...
    final formType = widget.hour != null ? 'hourly' : 'dailymetrics';
  ...
  Future<void> _submitForm() async {
    ...
    try {
      final dateString = DateFormat('yyyy-MM-dd').format(widget.selectedDate);
      final hourString = widget.hour?.toString() ?? '0';
      final entryId = widget.existingEntryId ?? 
          'entry_${widget.projectId}_${dateString}_$hourString';
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              widget.hour != null 
                  ? 'Hourly reading saved successfully'
        ...
        // Navigate back or to next hour
        context.pop();
      }
    } catch (e) {
      setState(() {
        _isSaving = false;
        _saveError = 'Failed to save entry: $e';
      });
    }
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('Failed to load form template', 
        ...
        children: [
          ...
          Expanded(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_isSaving ? 'Saving...' : 'Submit Entry'),
