...
interface VariableEditModalProps {
  ...
  onClose: () => void;
  onSave: (updates: Partial<LogVariable>) => void;
...
export function VariableEditModal({
  ...
  useEffect(() => {
    const hasChanges = JSON.stringify(formData) !== JSON.stringify(variable);
  ...
  const handleSubmit = () => {
    ...
    const updates: Partial<LogVariable> = {};
    const keys: (keyof LogVariable)[] = [
      'name', 'category', 'dataType', 'description', 'excelColumn',
      'tags', 'validation', 'options', 'defaultValue', 'unit',
      'formula', 'dependsOn'
    ];
    
    keys.forEach(key => {
      if (JSON.stringify(formData[key]) !== JSON.stringify(variable[key])) {
            ...
            <TabsContent value="advanced" className="space-y-4 mt-4">
              ...
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {variable.lastUsed && (
                    <div>Last Used: {new Date(variable.lastUsed).toLocaleDateString()}</div>
                  )}
                  <div>Favorite: {variable.isFavorite ? 'Yes' : 'No'}</div>
                </div>
              </div>
            </TabsContent>
          </Tabs>
          ...
          <DialogFooter className="mt-6">
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleSubmit} disabled={!isDirty} aria-label="Save changes">
                  Save Changes
                </Button>
              </div>
            </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDelete} aria-label="Confirm delete">
              Delete Variable
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
